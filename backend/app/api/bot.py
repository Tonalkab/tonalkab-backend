...
def chatear_con_bot(
    ...
    if provider == "gemini":
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail="Error procesando la solicitud con Gemini.")
    ...
    # =========================================================
        ...
        try:
            ...
            if request.imagen_base64:
                ...
                response = client.chat.completions.create(model=modelo_groq, messages=messages, temperature=0.6)
                ...
                tools_groq = [
                    {
                        "type": "function",
                        "function": {
                            "name": "herramienta_ver_estado_plantas",
                            "description": "Consulta el estado actual de los sensores de las macetas del usuario para informarle."
                        }
                    },
                    {
                        "type": "function",
                        "function": {
                            ...
                            "parameters": {
                                "type": "object",
                                "properties": {
                                    "id_maceta": {"type": "integer"},
                                    "mililitros": {"type": "number"}
                                },
                                "required": ["id_maceta", "mililitros"]
                            }
                        }
                    }
                ]
                ...
                if response_message.tool_calls:
                    ...
                    for tool_call in response_message.tool_calls:
                        ...
                        resultado_ejecucion = ""
                        if func_name == "herramienta_ver_estado_plantas":
                            resultado_ejecucion = herramienta_ver_estado_plantas()
                        elif func_name == "herramienta_encender_bomba":
                            resultado_ejecucion = herramienta_encender_bomba(
                                id_maceta=int(func_args.get("id_maceta")),
                                mililitros=float(func_args.get("mililitros"))
                            )
                    ...
                    return BotChatResponse(respuesta=second_response.choices[0].message.content)

                return BotChatResponse(respuesta=response_message.content)
